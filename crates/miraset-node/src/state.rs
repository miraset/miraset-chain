...
use chrono::Utc;
use miraset_core::{
    Address, Block, Event, JobStatus, Object, ObjectData, ObjectId, Transaction, Version,
    WorkerStatus, new_object_id,
};
...
impl State {
    ...
    fn execute_transaction_inner(
        ...
        match tx {
            ...
            Transaction::RegisterWorker {
                ...
                w.current_epoch
                    .worker_stats
                    .insert(worker_id, WorkerEpochStats::new(worker_id, *owner));
            ...
            Transaction::CreateJob {
                ...
                // Create job object
                let job_id =
                    new_object_id(&bincode::serialize(&(requester, model_id, Utc::now()))?);
    ...
    fn emit_event(&self, w: &mut StateInner, event: Event) -> Result<(), StateError> {
